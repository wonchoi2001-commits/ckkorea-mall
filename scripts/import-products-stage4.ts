...
function pickAvailableColumns(
  product: NormalizedStage4Product,
...
function buildPayload(product: NormalizedStage4Product, availableColumns: Set<string>) {
...
async function main() {
  ...
  const normalizedContent = JSON.parse(
    await fs.readFile(path.join(outputDir, "normalized_products_stage4.json"), "utf8")
  ) as {
    products: NormalizedStage4Product[];
  ...
  const result = {
    ...
    sourceFile: "normalized_products_stage4.json",
    ...
    categoryCounts: normalizedContent.meta.categoryCounts ?? {},
    featuredCandidates: normalizedContent.featuredProducts ?? [],
  };

  await fs.writeFile(
    path.join(outputDir, "import_result_stage4.json"),
    JSON.stringify(result, null, 2),
    "utf8"
  );

  console.log(
    `stage4 상품 import 완료: ${normalizedContent.products.length}개 처리, 신규 ${inserted}개, 업데이트 ${updated}개, 총 DB ${count ?? "확인불가"}개`
  );
}

main().catch((error: unknown) => {
  console.error("import-products-stage4 실패:", error);
  process.exitCode = 1;
});
