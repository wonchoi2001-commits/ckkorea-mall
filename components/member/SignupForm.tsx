...
export default function SignupForm() {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <label className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4 text-sm font-semibold text-slate-800">
        <input
          type="checkbox"
          checked={receiveMarketing}
          onChange={(event) => setReceiveMarketing(event.target.checked)}
        />
        배송/행사/신규 품목 안내를 받고 싶습니다
      </label>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 text-sm text-slate-500">
        ...
        <Link href="/business-benefits" className="font-semibold text-slate-700 hover:text-slate-950">
          사업자회원 혜택 보기
        </Link>
      </div>
    </form>
